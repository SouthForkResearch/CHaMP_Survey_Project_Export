...
def run(directorySource, directoryOutput, outLogFile, filterCSVFile=""):
    ...
    for directoryCurrent in os.listdir(directorySource):
        listsurveyGDB = glob.glob(os.path.join(directoryCurrent, "survey.gdb"))
        if len(listsurveyGDB) == 1:
            SurveyGDB = CHaMP_Data.SurveyGeodatabase(listsurveyGDB[0])
            outputFolder = os.path.join(directoryCurrent, "GISLayers")
            ...
            printer("   " + str(directoryCurrent) + ": Data Incomplete", outLogFile)
    else:
        printer("   Visit " + str(directoryCurrent) + " not run due to filter.", outLogFile)
