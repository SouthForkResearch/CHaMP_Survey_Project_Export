...
class TopoProject():
    ...
    LAYERS = {
        ...
        "Survey_Extent": "./Realizations/SurveyData[@projected='true']/SurveyExtents/Vector[@id='survey_extent']/Path",
        "ChannelUnitsField": "./Realizations/Topography/TIN[@active='true']/ChannelUnitsField/Path"} #KMW
    ...
    def __init__(self, sProjPath):
        ...
        if path.isfile(sProjPath):
            ...
            raise Exception("No project file or directory with the name could be found: {}".format(sProjPath))
        ...
        if path.isfile(self.projpathxml):
            ...
            try:
                ...
                raise Exception("project.rs.xml exists but could not be parsed.")
    ...
    def getpath(self, layername):
        ...
        if layername not in TopoProject.LAYERS:
            raise Exception("'{}' is not a valid layer name".format(layername))

        try:
            node = self.domroot.find(TopoProject.LAYERS[layername]).text.replace("\\", path.sep).replace("/", path.sep)
        except Exception, e:
            #raise Exception("Error retrieving layer '{}' from project file.".format(layername))
            return None
        if node is not None:
            ...
            raise Exception("Could not find layer '{}' with xpath '{}'".format(layername, TopoProject.LAYERS[layername]))
    ...
    def getMeta(self, metaname):
        ...
        try:
            ...
            raise Exception("Error retrieving metadata with name '{}' from project file.".format(metaname, self.projpathxml))
